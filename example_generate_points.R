# This example file generates a two-dimensional dataset of 300 points.
...
### Generating example data: 6 Normal Clusters
### This code produces a matrix A of points
dim <- 2 # number of dimensions
cen <- 6 # number of clusters
s <- matrix(0.003,cen) # matrix of standard deviations
cl <- 50 # cluster size
n1 <- matrix(cl,cen)
...
if (dim == 2) {
  plot(A, pch=20, main="Six Normal Clusters")
}
...
beta <- 0.5 # tuning parameter for tpPL method (0.3-0.8 range recommended)
tppl <- d+beta*tp # tree-penalized path length matrix as a "dist" object
